...
KWS 与 ASR 的模型许可证状态明确为 **unresolved**。默认 provision、verify 与 release 检查会在创建目录或下载前 fail-closed；
在取得权威的权重及训练数据许可前，不能进入 KWS/ASR Runtime。VAD 的 `MIT` 是 Silero 上游模型 provenance，和
